...
interface ReadyCopperSale {
  _id: string;
  readyCopperInventoryId: string;
  originalReadyCopperId: string | { _id: string; [key: string]: any };
  itemName: string;
  quantity: number;
  pricePerUnit: number;
  totalAmount: number;
...
interface GetReadyCopperSalesResponse {
  success: boolean;
  data: ReadyCopperSale[];
  pagination: {
    page: number;
    limit: number;
    total: number;
...
interface SellReadyCopperProps {
  onDataChange?: () => void;
}

const SellReadyCopper = ({ onDataChange }: SellReadyCopperProps) => {
  const { toast } = useToast();
  const [returnedInventory, setReturnedInventory] = useState<ReadyCopperInventoryItem[]>([]);
  const [salesHistory, setSalesHistory] = useState<ReadyCopperSale[]>([]);
  const [loading, setLoading] = useState(true);
  const [deletingSaleId, setDeletingSaleId] = useState<string | null>(null);

  // Sell Ready Copper modal state
  const [sellModalOpen, setSellModalOpen] = useState(false);
  const [sellLoading, setSellLoading] = useState(false);

  // Delete confirmation modal state
  const [deleteConfirmModalOpen, setDeleteConfirmModalOpen] = useState(false);
  const [saleToDelete, setSaleToDelete] = useState<ReadyCopperSale | null>(null);

  // react-hook-form for Sell Ready Copper
  ...
  // Fetch returned ready copper inventory
  ...
  const fetchSalesHistory = async () => {
    try {
      const response = await request<void, GetReadyCopperSalesResponse>({
        url: '/ready-copper/sales',
        method: 'GET',
      });
      if (response.success) {
  ...
  // Sell Ready Copper handler
  const onSellReadyCopper = async (data: SellFormData) => {
    try {
      setSellLoading(true);
      const response = await request<any, { success: boolean; message: string }>({
        url: '/ready-copper/inventory/sell',
        method: 'POST',
        data: {
      ...
      if (response.success) {
        ...
        toast({ title: 'Error', description: response.message || 'Failed to sell ready copper.', variant: 'destructive' });
      }
    } catch (error: any) {
      toast({ title: 'Error', description: error.message || 'Failed to sell ready copper.', variant: 'destructive' });
    } finally {
  ...
  const confirmDeleteSale = async () => {
    ...
    try {
      ...
      const response = await request<any, { success: boolean; message: string }>({
        url: `/ready-copper/sales/${saleToDelete._id}`,
        method: 'DELETE',
      });
      if (response.success) {
        toast({ title: 'Success', description: response.message });
  ...
  return (
    <Card className="mt-6">
      <CardHeader>
        <CardTitle className="text-lg font-sans">Ready Copper Sales History</CardTitle>
        <p className="text-gray-500 dark:text-gray-400 text-sm">
          Record of all ready copper sales
        </p>
        <div className="flex w-full justify-end gap-2 mt-4">
          <Dialog open={sellModalOpen} onOpenChange={setSellModalOpen}>
            ...
            <DialogContent className="sm:max-w-[400px]">
              <DialogHeader>
                <DialogTitle>Sell Ready Copper</DialogTitle>
              </DialogHeader>
              <form onSubmit={handleSellSubmit(onSellReadyCopper)} className="space-y-4">
                <div>
                          ...
                          <SelectContent>
                            {returnedInventory.map((inv) => (
                              <SelectItem key={inv._id} value={inv._id}>
                                {inv.newName || inv.originalReadyCopperId.productName} (Available: {inv.quantity})
                              </SelectItem>
                            ))}
                          </SelectContent>
                        </Select>
                    ...
                    <td className="px-4 py-2">
                      <div>
                        ...
                        <span className="text-xs text-gray-500 block">
                          (Originally: {typeof sale.originalReadyCopperId === 'object' ? sale.originalReadyCopperId._id || 'N/A' : sale.originalReadyCopperId || "N/A"})
                        </span>
                      </div>
                    </td>
...
export default SellReadyCopper; 